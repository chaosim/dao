...
- optional(item, mode=nongreedy):
  可选匹配符。也可写成may(item, item)。有贪婪和非贪婪两种工作模式。贪婪模式下，item匹配成功后不会may(item, mode)命令不会回溯。非贪婪模式下，可以回溯，当发生回溯时，may(item, nongreedy)的作用等同于空字符nullword。
...
- Repeater(item, separator=None, min=None, max=None, template=None, result=None, mode=nongreedy)
  ...
  - 非贪婪模式(nongreedy): matcher[:]/separator%template*result，默认的工作模式，先尽量多匹配字符，如果后续命令失败，则发生回溯，减少重复次数，回退解析位置，然后重新执行后续命令。
  
  - 贪婪模式(greedy): +matcher[:]/separator%template*result，尽量多匹配字符，即是后续命令失败，也不会这个重复匹配符上发生回溯，不会回退解析位置。
