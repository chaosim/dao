...
def get_first_set(command, solver, memo):
  try: command_first_set = command.first_set
  except: return uniset
  return command_first_set(solver, memo)

def exps_first_set(self, solver, memo, *exps):
  result = set()
  for exp in exps:
    fs = get_first_set(exp, solver, memo)
    result = result|fs
    if null not in fs: break
  return result
