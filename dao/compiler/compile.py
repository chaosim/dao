...
#¦Á-conversion
#Alpha-conversion, sometimes known as alpha-renaming,[11] allows bound variable names to be changed. For example, alpha-conversion of ¦Ëx.x might yield ¦Ëy.y. Terms that differ only by alpha-conversion are called ¦Á-equivalent. Frequently in uses of lambda calculus, ¦Á-equivalent terms are considered to be equivalent.
#The precise rules for alpha-conversion are not completely trivial. First, when alpha-converting an abstraction, the only variable occurrences that are renamed are those that are bound to the same abstraction. For example, an alpha-conversion of ¦Ëx.¦Ëx.x could result in ¦Ëy.¦Ëx.x, but it could not result in ¦Ëy.¦Ëx.y. The latter has a different meaning from the original.
#Second, alpha-conversion is not possible if it would result in a variable getting captured by a different abstraction. For example, if we replace x with y in ¦Ëx.¦Ëy.x, we get ¦Ëy.¦Ëy.y, which is not at all the same.
