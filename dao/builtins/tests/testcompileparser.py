...
from dao.compile import compile, compile_exec as compile_exec0, compile2file
...
def compile_exec(exp):
  compile_exec0(exp, globals())
  
class xTestLowLevelPrimitive:
  def test_step(self):
...
class Testterminal:
  def test_char(self):
    eq_(compile(parse_text(char('a'), 'a')), 'a')
  def test_compile_eval_char(self):
    eq_(compile_exec(parse_text(char('a'), 'a')), 'a')
  def test_compile2file(self):
    compile2file(parse_text(char('a'), 'a'), 'char1.py')
